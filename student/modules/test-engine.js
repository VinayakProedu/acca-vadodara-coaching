/* ============================================================
   Phase 3B — Test Engine (MCQ, Multi, Fill, Drag, Dropdown,
   Hotspot, OT Case, Word/Sheet + Scoring + Timer + Results)
   ============================================================ */
(function(){
  'use strict';
  ...
  const els = {
    title: $('overlayTitle'), progress: $('overlayProgress'), bar: $('progressBar'),
    area: $('questionArea'), feedback: $('questionFeedback'), timer: $('testTimer'),
    prev: $('prevBtn'), next: $('nextBtn'), submit: $('submitBtn'), close: $('closeTestOverlayBtn')
  };
  ...
  function resetAnswers(){
    S.choice = null; S.multi = []; S.dropdown = ''; S.fill = '';
    S.manual = ''; S.hotspotText = ''; S.hotspotClick = null; S.dragMap = {};
    S.submitted = false;
  }
  ...
  function setProgress(){
    ...
    els.progress.textContent = `Q ${cur} / ${total}`;
    els.bar.style.width = `${(cur/total)*100}%`;
  }
  function clearFeedback(){ els.feedback.classList.add('hidden'); els.feedback.className='feedback hidden'; }
  function setFeedback(html, kind){
    els.feedback.classList.remove('hidden');
    els.feedback.innerHTML = html;
    els.feedback.classList.add(kind==='correct'?'correct':kind==='wrong'?'wrong':'');
  }
  function setActionState(){
    if(S.finished){
      els.prev.style.display = 'none';
      els.next.textContent = 'Close'; els.next.style.display = 'inline-flex';
    ...
    if(S.submitted){
      ...
      els.next.textContent = last ? 'Finish' : 'Next →'; els.next.style.display = 'inline-flex';
      els.submit.style.display = 'none';
    } else {
  ...
  function renderQuestion(){
    const q = currentQ(); if(!q){ els.area.innerHTML=''; return; }
    ...
    let html = `<div class="question-card" id="questionCard">
      <div class="question-meta">
        <div><h3 class="question-title">Question ${S.index+1}</h3>
        <div class="question-sub">${esc(S.test.title||'')}${S.test.subject?' · '+esc(S.test.subject):''}</div></div>
    ...
    if(type==='mcq'){
      html += `<div>${(q.options||[]).map((opt,i)=>`
        <button type="button" class="option-btn ${S.choice===i?'selected':''}" onclick="window.__pickSingle(${i})">${esc(opt)}</button>`).join('')}</div>`;
    }
    else if(type==='dropdown'){
      html += `<select class="dropdown-select" onchange="window.__setDropdown(this.value)">
        <option value="">— Choose an option —</option>
        ${(q.options||[]).map((opt,i)=>`<option value="${i}" ${String(S.dropdown)===String(i)?'selected':''}>${esc(opt)}</option>`).join('')}
      </select>`;
    }
    else if(type==='multi'){
      html += `<div>${(q.options||[]).map((opt,i)=>`
      ...
      html += `<div class="drag-wrap">
        <div class="drag-box"><strong style="color:#0D1B40;display:block;margin-bottom:8px;">Drag items</strong>
          <div>${pairs.map(p=>`<div class="drag-item" draggable="true" data-drag="${esc(p.drag)}">${esc(p.drag)}</div>`).join('')}</div>
        </div>
        <div class="drop-box"><strong style="color:#0D1B40;display:block;margin-bottom:8px;">Drop targets</strong>
  ...
  function captureHotspot(e){
    ...
    S.hotspotClick = {x:Math.round(e.clientX-rect.left), y:Math.round(e.clientY-rect.top)};
  ...
  function submitAnswer(){
    ...
    if(type==='mcq'){
      ...
      feedback = isCorrect?`✅ Correct${q.explanation?'<br><br><strong>Explanation:</strong> '+esc(q.explanation):''}`
      ...
      const pairs=Array.isArray(q.pairs)?q.pairs:[];
      isCorrect=true;
      pairs.forEach(p=>{ if(String(S.dragMap[String(p.drop)]||'')!==String(p.drag)) isCorrect=false; });
      if(isCorrect) S.score++;
      ...
      if(S.hotspotClick&&exp){
        ...
        if(m){ const ex={x:Number(m[1]),y:Number(m[3])}; same=Math.abs(S.hotspotClick.x-ex.x)<=25&&Math.abs(S.hotspotClick.y-ex.y)<=25; }
      } else if(typed){ same=typed.toLowerCase()===exp.toLowerCase(); }
      isCorrect=same; if(same) S.score++;
      userText=typed||`${S.hotspotClick?.x}, ${S.hotspotClick?.y}`;
      feedback=same?`✅ Submitted${q.explanation?'<br><br><strong>Explanation:</strong> '+esc(q.explanation):''}`
  ...
  function nextQ(){
    ...
    if(S.index < S.questions.length){
      resetAnswers(); clearFeedback(); renderQuestion();
    } else {
      finishTest();
    }
  }
  function prevQ(){
    ...
    if(S.index>0){ S.index--; resetAnswers(); clearFeedback(); renderQuestion(); }
  }

  /* ── finish & save ── */
  async function saveAttempt(){
    const user=currentUser(); if(!user) throw new Error('Not logged in');
    const total=S.questions.length, score=S.score;
    await db().collection('attempts').add({
      userId:user.uid, userEmail:user.email||'', userName:user.displayName||user.email||'Student',
      testId:S.test.id, testTitle:S.test.title||'Test', subject:S.test.subject||'General', chapter:S.test.chapter||'General',
      score:Number(score||0), total:Number(total||0),
      percentage: Number(total?Math.round((score/total)*100):0),
      timeTaken:S.timeTaken, responses:S.responses,
  ...
  async function finishTest(){
    ...
    try{ await saveAttempt(); }catch(e){ console.error(e); }

    const total=S.questions.length, score=S.score, pct=total?Math.round((score/total)*100):0;
    els.area.innerHTML = `<div class="question-card">
    ...
    els.prev.style.display='none'; els.submit.style.display='none';
    els.next.textContent='Close'; els.next.style.display='inline-flex';
    els.progress.textContent='Done'; els.bar.style.width='100%';
  ...
  function closeOverlay(){
    ...
    clearInterval(S.timerId);
    S={test:null,questions:[],index:0,score:0,responses:[],startTime:null,timeTaken:0,timerId:null,
      choice:null,multi:[],dropdown:'',fill:'',manual:'',hotspotText:'',hotspotClick:null,dragMap:{},
      submitted:false,finished:false};
    els.area.innerHTML=''; els.feedback.className='feedback hidden';
  ...
  window.__startTest = async function(testId){
    try{
      ...
      const questions = qSnap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(a.createdAt?.seconds||0)-(b.createdAt?.seconds||0));
      ...
      S.test = {
        ...(testDoc.data?testDoc.data():{}),
        id:testId,
        title:(testDoc.data&&testDoc.data().title)||firstQ.testTitle||firstQ.title||'Practice Test',
        subject:(testDoc.data&&testDoc.data().subject)||firstQ.subject||'General',
        chapter:(testDoc.data&&testDoc.data().chapter)||firstQ.chapter||'General'
      };
      S.questions = questions;
      S.index=0; S.score=0; S.responses=[]; resetAnswers();

      els.title.textContent = S.test.title||'Practice Test';
      overlay.classList.add('show');
      document.body.style.overflow='hidden';
      renderQuestion();
      startTimer();
    }catch(err){console.error(err);alert('Error loading test: '+err.message);}
  };
